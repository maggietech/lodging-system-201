...
export async function registerGuest(guest) {
    ...
    return window.canister.house.addGuest(guest.id, guest.name);
}

// Function to update a guest
export async function updateGuest(guest) {
    // Call the updateGuest function from the canister
    return window.canister.house.updateGuest(guest.id, guest.name);
}
...
export async function getGuestDetails(id) {
    try {
        ...
        return await window.canister.house.getGuestDetails(id);
    } catch (err) {
        // Handle errors
